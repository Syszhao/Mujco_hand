...
class RotateBallsEnv(gym.Env):
    ...
    def _compute_reward(self) -> float:
        ...
        try:
            ...
            height_penalty = 0.0
            # 给予高度太高或太低的惩罚
            if ball1_pos[2] < 0.2 or ball1_pos[2] > 0.9:
                height_penalty += 15.0
            if ball2_pos[2] < 0.2 or ball2_pos[2] > 0.9:
                height_penalty += 15.0
            ...
            # 奖励裁剪
            reward = np.clip(reward, -150.0, 3000.0)  # 扩大奖励范围
            return float(reward)
        except Exception as e:
            print(f"计算奖励时出错: {e}")
            import traceback
        ...
        try:
            ...
            if (self.ball1_id != -1 and ball1_height < ground_height) or \
               (self.ball2_id != -1 and ball2_height < ground_height):
                 ...
                 termination_penalty = -50.0  # 增加掉落惩罚
...
def main():
    ...
    parser.add_argument('--total-steps', type=int, default=1000000, help='总训练步数')
    ...
    parser.add_argument('--num-envs', type=int, default=8, help='并行环境数量')
