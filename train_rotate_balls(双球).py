...
class RotateBallsEnv(gym.Env):
    ...
    def _compute_reward(self) -> float:
        ...
        try:
            ...
            # 球体在手掌上的奖励
            if ball1_on_palm:
                palm_reward1 = 30.0 * np.exp(-5.0 * abs(ball1_pos[2] - ideal_height))
                reward += palm_reward1
            else:
                # 不在手掌上的惩罚，距离越远惩罚越大
                palm_penalty1 = -100.0 * (1.0 - np.exp(-3.0 * max(0, ball1_xy_dist - palm_radius)))
                reward += palm_penalty1
                
            if ball2_on_palm:
                palm_reward2 = 30.0 * np.exp(-5.0 * abs(ball2_pos[2] - ideal_height))
                reward += palm_reward2
            else:
                # 不在手掌上的惩罚，距离越远惩罚越大
                palm_penalty2 = -100.0 * (1.0 - np.exp(-3.0 * max(0, ball2_xy_dist - palm_radius)))
                reward += palm_penalty2
            
            # 球体高度过低的严厉惩罚，防止掉落
            fall_threshold = palm_height - 0.05  # 低于手掌一定高度视为要掉落
            if ball1_pos[2] < fall_threshold:
                fall_penalty = -1000.0 * (fall_threshold - ball1_pos[2]) * 20.0  # 极大增加惩罚力度
                reward += fall_penalty
                # print(f"球1高度过低警告: {ball1_pos[2]:.3f}, 惩罚: {fall_penalty:.1f}")
                
            if ball2_pos[2] < fall_threshold:
                fall_penalty = -1000.0 * (fall_threshold - ball2_pos[2]) * 20.0  # 极大增加惩罚力度
                reward += fall_penalty
                # print(f"球2高度过低警告: {ball2_pos[2]:.3f}, 惩罚: {fall_penalty:.1f}")
                
            # 球体离开手掌区域的严厉惩罚
            if not ball1_on_palm:
                off_palm_penalty = -300.0 * (1.0 - np.exp(-5.0 * max(0, ball1_xy_dist - palm_radius)))
            ...
            if not ball2_on_palm:
                off_palm_penalty = -300.0 * (1.0 - np.exp(-5.0 * max(0, ball2_xy_dist - palm_radius)))
                reward += off_palm_penalty
            ...
            if self.last_order is not None and current_order != self.last_order and y_diff > 0.01:  # 添加最小距离要求
                ...
                if hasattr(self, '_real_exchange_check') and self._real_exchange_check:
                    ...
                    if ball1_on_palm and ball2_on_palm:
                        exchange_reward = 3000.0  # 大幅提高交换奖励
                        self.exchange_count += 1
                        reward += exchange_reward
                        if self.exchange_count > 1:
                            combo_reward = min(800.0 * (self.exchange_count - 1), 3000.0)  # 增加连续交换奖励
                            reward += combo_reward
            ...
            control_reward1 = 20.0 * np.exp(-10.0 * abs(dist1 - optimal_dist))
            control_reward2 = 20.0 * np.exp(-10.0 * abs(dist2 - optimal_dist))
            ...
            if crossing_direction and ball1_on_palm and ball2_on_palm:
                cross_reward = 200.0 * np.exp(-15.0 * y_diff)  # 距离越近奖励越高，增加奖励力度
                reward += cross_reward
                
                # 临界交叉奖励
                if y_diff < 0.02:
                    reward += 300.0  # 增加临界交叉奖励
            ...
            if len(self.actuator_ids) > 0:
                ...
                if joint_velocities:
                    ...
                    coordination_reward = 15.0 * np.exp(-5.0 * abs(avg_joint_vel - optimal_joint_vel))
            ...
            if hasattr(self, '_last_ball1_pos') and hasattr(self, '_last_ball2_pos'):
                ...
                if abs(curr_rel_pos) < abs(last_rel_pos):
                    ...
                    # 当两球都在手掌上时，增加朝交叉方向移动的奖励力度
                    if ball1_on_palm and ball2_on_palm:
                        rel_motion_reward = 100.0 * rel_pos_change  # 加倍朝交叉方向移动的奖励
                    else:
                        rel_motion_reward = 50.0 * rel_pos_change
                    reward += rel_motion_reward
                    
                    # 当相对位置接近零或改变符号时，给予更大额外奖励
                    if curr_rel_pos * last_rel_pos <= 0:  # 符号变化或为零
                        reward += 150.0  # 增加接近交换位置的奖励
                        print("球体相对位置发生符号变化，接近交换!")
                else:
                    # 对远离交叉的移动增加惩罚，但只在球体都在手掌上时
                    if ball1_on_palm and ball2_on_palm:
                        rel_pos_change = abs(abs(curr_rel_pos) - abs(last_rel_pos))
                        reward -= 30.0 * rel_pos_change  # 增加惩罚力度
            
            # 新增：Y轴方向交换趋势奖励 - 更强调两球Y方向的相对运动
            if hasattr(self, '_last_ball1_pos') and hasattr(self, '_last_ball2_pos'):
                # 计算两球Y方向的位置变化
                ball1_y_change = ball1_y - self._last_ball1_pos[1]
                ball2_y_change = ball2_y - self._last_ball2_pos[1]
                
                # 球体都在手掌上，给予朝着交换方向移动的额外奖励
                if ball1_on_palm and ball2_on_palm:
                    # 如果球1在球2右侧(ball1_y > ball2_y)，则球1应向左移动(ball1_y_change < 0)，球2应向右移动(ball2_y_change > 0)
                    # 如果球1在球2左侧(ball1_y < ball2_y)，则球1应向右移动(ball1_y_change > 0)，球2应向左移动(ball2_y_change < 0)
                    if (ball1_y > ball2_y and ball1_y_change < 0) or (ball1_y < ball2_y and ball1_y_change > 0):
                        # 球1朝着交换方向移动
                        exchange_direction_reward1 = 30.0 * abs(ball1_y_change)
                        reward += exchange_direction_reward1
                    
                    if (ball1_y > ball2_y and ball2_y_change > 0) or (ball1_y < ball2_y and ball2_y_change < 0):
                        # 球2朝着交换方向移动
                        exchange_direction_reward2 = 30.0 * abs(ball2_y_change)
                        reward += exchange_direction_reward2
                    
                    # 两球同时朝着交换方向移动的额外奖励
                    if ((ball1_y > ball2_y and ball1_y_change < 0 and ball2_y_change > 0) or 
                        (ball1_y < ball2_y and ball1_y_change > 0 and ball2_y_change < 0)):
                        simultaneous_move_reward = 50.0 * (abs(ball1_y_change) + abs(ball2_y_change))
                        reward += simultaneous_move_reward
            ...
            if ball1_pos[2] > max_height:
                height_penalty += 50.0 * (ball1_pos[2] - max_height) * 10.0
                print(f"球1高度过高: {ball1_pos[2]:.3f}, 惩罚: {50.0 * (ball1_pos[2] - max_height) * 10.0:.1f}")
            
            if ball2_pos[2] > max_height:
                height_penalty += 50.0 * (ball2_pos[2] - max_height) * 10.0
                print(f"球2高度过高: {ball2_pos[2]:.3f}, 惩罚: {50.0 * (ball2_pos[2] - max_height) * 10.0:.1f}")
            
            # 更强的高度维持奖励 - 使用前面定义的理想高度
            height_reward = 30.0 * (np.exp(-8.0 * abs(ball1_pos[2] - ideal_height)) + 
            ...
            same_height_reward = 40.0 * np.exp(-10.0 * height_diff)
            ...
            if abs(ball1_pos[2] - ideal_height) < 0.1:
                z_vel_reward1 = 15.0 * np.exp(-5.0 * abs(ball1_z_vel))
                reward += z_vel_reward1
            
            if abs(ball2_pos[2] - ideal_height) < 0.1:
                z_vel_reward2 = 15.0 * np.exp(-5.0 * abs(ball2_z_vel))
                reward += z_vel_reward2
            
            # 如果高度太高，鼓励向下运动
            if ball1_pos[2] > ideal_height + 0.1 and ball1_z_vel < 0:
                reward += 10.0 * abs(ball1_z_vel)  # 向下运动得到奖励
            
            if ball2_pos[2] > ideal_height + 0.1 and ball2_z_vel < 0:
                reward += 10.0 * abs(ball2_z_vel)  # 向下运动得到奖励
            
            # 如果高度太低，鼓励向上运动
            if ball1_pos[2] < ideal_height - 0.1 and ball1_z_vel > 0:
                reward += 10.0 * ball1_z_vel  # 向上运动得到奖励
            
            if ball2_pos[2] < ideal_height - 0.1 and ball2_z_vel > 0:
                reward += 10.0 * ball2_z_vel  # 向上运动得到奖励
                
            # 新增：两球相互靠近时的交换意图奖励
            if y_diff < 0.06 and ball1_on_palm and ball2_on_palm:
                # 两球Y轴距离很近时，额外给予交换动作的激励
                proximity_exchange_reward = 100.0 * (0.06 - y_diff) / 0.06
                reward += proximity_exchange_reward
                
                # 如果两球非常接近，给予更大奖励鼓励最后一步的交换
                if y_diff < 0.02:
                    reward += 200.0
                    print(f"两球非常接近，强烈鼓励交换行为! y_diff={y_diff:.4f}")
            ...
            # 奖励裁剪
            reward = np.clip(reward, -3000.0, 3000.0)  # 保持惩罚和奖励的平衡
            return float(reward)
        except Exception as e:
            print(f"计算奖励时出错: {e}")
            import traceback
...
def main():
    ...
    parser.add_argument('--total-steps', type=int, default=500000, help='总训练步数')
